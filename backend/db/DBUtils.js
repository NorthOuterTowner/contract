...
const db = {
  async: {
    ...
    // 获取下一个可用的角色 ID
    getNextRoleId: () => {
      const sql = "SELECT AUTO_INCREMENT FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'Roles'";
      return db.async.all(sql, []);
    },

    // 添加角色
    addRole: (roleName, roleDescription, permissions) => {
      return new Promise(async (resolve, reject) => {
        try {
          // 插入角色
          const roleResult = await db.async.run(
            "INSERT INTO Roles (RoleName, RoleDescription) VALUES (?, ?)",
            [roleName, roleDescription]
          );
          const roleId = roleResult.result.insertId;
          ...
          resolve({ err: null, result: roleResult.result });
        } catch (error) {
          reject(error);
        }
      });
    ...
    queryRoles: (roleName) => {
      ...
      return db.async.all(sql, params);
    },

    // 修改角色
    updateRole: (roleId, roleName, roleDescription, permissions) => {
      return new Promise(async (resolve, reject) => {
        try {
          // 更新角色信息
